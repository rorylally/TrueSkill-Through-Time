# A tibble: 1 × 11
#model                 gap_days alpha  beta n_matches ttt_acc book_acc inj_rate n_inj ttt_acc_inj book_acc_inj
#<chr>                    <dbl> <dbl> <dbl>     <int>   <dbl>    <dbl>    <dbl> <int>       <dbl>        <dbl>
#1 Absence-adjusted TTT…       16   0.5     1      1132    64.0     68.4     64.6   731        66.5         71.3



# ============================
# FINAL MODEL (FROZEN)
# Absence-adjusted TTT using skills_snapshot_by_ATP.csv
# Matches your tuning logic exactly (tournament anchor + expanding train pool)
# ============================
...
inj_subset <- comp_df %>%
  filter(inj_w == 1L | inj_l == 1L)

final_results <- tibble(
  model              = "Absence-adjusted TTT (final)",
  gap_days           = GAP_DAYS,
  alpha              = ALPHA,
  beta               = BETA,
  ...
  inj_rate           = round(100 * mean(comp_df$inj_w == 1L | comp_df$inj_l == 1L), 2),
  n_inj              = nrow(inj_subset),
  ttt_acc_inj        = round(mean(inj_subset$ttt_correct) * 100, 2),
  book_acc_inj       = round(mean(inj_subset$bookie_correct) * 100, 2)
)
