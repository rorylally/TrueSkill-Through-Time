#Accuracy — TTT: 62.54% | TTT+Fatigue: 62.81% | Bookmakers: 68.37% (same 1132 matches)
...
# 0) SETTINGS
# ============================================================
MAX_SETS    <- 5        # change to 3 if you only have W1..W3/L1..L3
DELTA       <- 0.75     # Sipko decay
WINDOW_DAYS <- 3        # Sipko window
EPS         <- 1e-6     # numerical stability for qlogis()

# ============================================================
# 1) HELPERS
# ============================================================

# Total games in match from set columns (W1/L1, W2/L2, ...)
...
# Build Sipko fatigue table: Fatigue(Player, Date)
...
# Given mu/sigma for winner & loser, return p_ttt = P(Winner beats Loser)
p_ttt_from_params <- function(mu_w, mu_l, s_w, s_l) {
  z <- (mu_w - mu_l) / sqrt(pmax(s_w^2 + s_l^2, 1e-9))
  pnorm(z)
}

# Fit theta via offset logistic regression on training matches
fit_theta_offset <- function(train_matches_with_probs_and_fat) {
  df_fit <- train_matches_with_probs_and_fat %>%
  ...
  # We want: logit(p) = logit(p_ttt) - theta * fat_diff
  # but glm fits + beta * fat_diff, hence theta = -beta
  theta_hat <- -coef(fit)[["fat_diff"]]
  theta_hat
}

# ============================================================
# 2) LOAD DATA + PRECOMPUTE FATIGUE
# ============================================================
...
# Add GamesMatch from set columns
df0 <- df0 %>%
  mutate(GamesMatch = games_from_setcols(cur_data(), w_prefix = "W", l_prefix = "L", max_sets = MAX_SETS))

# Precompute fatigue for all players/dates (safe: only looks backwards in time)
fatigue_tbl <- build_fatigue_tbl(df0, delta = DELTA, window_days = WINDOW_DAYS)

# ============================================================
# 3) YOUR ORIGINAL ROLLING SETUP
# ============================================================
...
for (k in ks) {
  ...
  # ============================================================
  # 4) FIT THETA ON TRAIN_POOL (OFFSET GLM) USING CURRENT TTT PROBS
  # ============================================================
  
  train_with_params <- train_pool %>%
    ...
    # attach fatigue values for Winner/Loser on that Date
  ...
  # ============================================================
  # 5) PREDICT ON TEST_DF + APPLY FATIGUE CORRECTION
  # ============================================================
  ...
  preds <- test_df %>%
    ...
    mutate(
      ...
      # baseline TTT probability
      p_ttt_win = p_ttt_from_params(mu_winner, mu_loser, sigma_winner, sigma_loser),
      p_ttt_win = pmin(pmax(p_ttt_win, EPS), 1 - EPS),
      
      # Option A: fatigue-adjusted probability
...
# ============================================================
# 6) EVALUATION ON SAME COMPARABLE MATCHES
# ============================================================
comp_df <- all_df %>%
  filter(
...
write_csv(disagree_baseline, "bet365_right_ttt_wrong_BASELINE_with_probs.csv")
write_csv(disagree_fatigue,  "bet365_right_ttt_wrong_FATIGUE_optionA.csv")
...
write_csv(accuracy_summary, "accuracy_summary_FATIGUE_optionA.csv")

cat("Saved:\n")
cat(" - bet365_right_ttt_wrong_BASELINE_with_probs.csv\n")
cat(" - bet365_right_ttt_wrong_FATIGUE_optionA.csv\n")
cat(" - accuracy_summary_FATIGUE_optionA.csv\n")
