...
TARGET_ATP <- c(1, 2, 3, 4, 6, 7, 8, 9, 10, 11, 13, 14, 18, 21, 22, 23, 26, 27, 30, 31, 33, 34, 37, 38, 40, 41, 42, 43, 44, 45, 46, 50, 52, 53, 57, 58, 59, 63, 64)
...
print(ttt_table)
readr::write_csv(ttt_table, "ttt_accuracy_by_year_ATP250.csv")
...
# Save outputs
write_csv(disagree_df, "bet365_right_ttt_wrong_ATP250.csv")
write.csv(all_df_export, "Global_TTT_rolling_result_ATP250.csv", row.names = FALSE)
write_xlsx(all_df_export, "Global_TTT_rolling_result_ATP250.xlsx")
cat("Files written: Global_TTT_rolling_result_ATP250.[csv/xlsx] and bet365_right_ttt_wrong_ATP250.csv\n")
...
cat(sprintf("Accuracy — TTT: %.2f%% | Bet365: %.2f%% (same %d matches)\n",
            ttt_acc, book_acc, n_comp))
write_csv(acc_table, "bet365_directional_accuracy_ATP250.csv")