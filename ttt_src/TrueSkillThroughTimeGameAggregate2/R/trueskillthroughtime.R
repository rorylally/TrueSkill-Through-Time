...
mov_weight <- function(games_w, games_l, mov_w_scale, mov_w_cap){
  if (!is.finite(games_w) || !is.finite(games_l)) return(1.0)
  m <- pmax(games_w - games_l, 0)
  w <- 1.0 + mov_w_scale * sqrt(m)
...
Batch <- setRefClass("Batch",
                     fields = list(
                       ...
                       agents = "hash",
                       p_draw = "numeric",
                       use_mov_weight = "logical",
                       mov_w_scale    = "numeric",
                       mov_w_cap      = "numeric"
                     )
)
Batch$methods(
  initialize = function(composition, results = list() ,time = 0, agents = hash(), p_draw=P_DRAW,use_mov_weight = FALSE, mov_w_scale = NA_real_, mov_w_cap = NA_real_){
  ...
  iteration = function(from_ =1){
    for (e in seq(from_,length(events))){
      ...
      if (use_mov_weight && length(result) == 2){
        w_event <- mov_weight(result[1], result[2], mov_w_scale, mov_w_cap)
      }
...
History$methods(
  initialize = function(composition, results=list(), times=c(), priors=hash(),  mu=MU, sigma=SIGMA, beta=BETA, gamma=GAMMA, p_draw=P_DRAW, epsilon=EPSILON,  iterations=ITERATIONS,use_mov_weight = FALSE, mov_w_scale = NA_real_, mov_w_cap = NA_real_){
    " "
    if ((length(results)>0) & (length(composition) != length(results))){ stop("(length(results)>0) & (length(composition) != length(results))")}
  ...
  trueskill = function(composition, results, times){
    ...
    while (i <= size){
      ...
      if (length(results)>0){
        b = Batch(composition[o[seq(i,j)]], results[o[seq(i,j)]], t, agents, p_draw,use_mov_weight = use_mov_weight,
                  mov_w_scale    = mov_w_scale,
                  mov_w_cap      = mov_w_cap)
      }else{
        b = Batch(composition[o[seq(i,j)]], list(), t, agents, p_draw,use_mov_weight = use_mov_weight,
                  mov_w_scale    = mov_w_scale,
                  mov_w_cap      = mov_w_cap)
      }
