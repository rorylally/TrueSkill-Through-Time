...
bounds_from_setmargin <- function(m, b1, b2){
  if (m == 1) return(c(0,  b1))
  if (m == 2) return(c(b1, b2))
  if (m == 3) return(c(b2, Inf))
  stop("set margin must be 1,2,3")
}
...
#####
# Interval-truncated normal moments: X ~ N(mu, sigma^2) truncated to (a, b)
trunc_interval <- function(mu, sigma, a, b){
  alpha <- (a - mu) / sigma
  beta  <- (b - mu) / sigma
  ...
  # v = (phi(alpha) - phi(beta)) / Z
  v <- (phi_a - phi_b) / Z
  
  # SAFE terms for alpha*phi(alpha) and beta*phi(beta) when alpha/beta are +/-Inf
  alpha_phi_a <- if (is.finite(alpha)) alpha * phi_a else 0
  beta_phi_b  <- if (is.finite(beta))  beta  * phi_b else 0
  
  # u = (alpha*phi(alpha) - beta*phi(beta)) / Z
  u <- (alpha_phi_a - beta_phi_b) / Z
  
  # match package convention: sigma_trunc = sigma * sqrt(1 - w) where w = v^2 - u
  w <- v^2 - u
  
  mu_trunc    <- mu + sigma * v
  sigma_trunc <- sigma * sqrt(max(1 - w, 1e-12))
...
#' @name Game
#' @export
Game <- function(teams, result = vector(), p_draw = P_DRAW, set_margins = NULL, b1 = NA, b2 = NA){
  ...
  l_e = compute_likelihoods(teams, result, p_draw, set_margins = set_margins, b1 = b1, b2 = b2)
  
  g = new("Game",
          teams = teams,
          result = result,
          p_draw = p_draw,
          likelihoods = l_e$likelihoods,
          evidence = l_e$evidence)
...
graphical_model <- function(teams, result, p_draw, set_margins = NULL, b1 = NA, b2 = NA){
  r <- if (length(result)>0) result else seq(length(teams)-1,0)
  o <- sortperm(r, decreasing = TRUE)
  
  t <- vector('list', length(teams))
  d <- vector('list', length(teams)-1)
  
  tie    <- rep(NA, length(d))
  margin <- rep(NA, length(d))
  
  # interval bounds for ordinal (second-order) preference
  a_int <- rep(NA_real_, length(d))
  b_int <- rep(NA_real_, length(d))
  
  # team performances/messages
  for (e in seq(length(teams))){
    team_perf <- N00
    for (agent in teams[[o[e]]]) {
      team_perf <- team_perf + performance(agent)
    }
    t[[e]] <- team_messages(team_perf)
  }
  
  # performance differences between adjacent ranks
  ...
  # draw margins (original)
  for (e in seq(length(d))){
    if (p_draw == 0.0){
      margin[e] <- 0.0
    } else {
      betas <- 0
      for (agent in teams[[o[e]]])   betas <- betas + agent@beta^2
      for (agent in teams[[o[e+1]]]) betas <- betas + agent@beta^2
      margin[e] <- compute_margin(p_draw, sqrt(betas))
    }
  }
  
  # NEW: ordinal bounds based on set margins (only if provided)
  if (!is.null(set_margins)) {
    for (e in seq_along(d)) {
      ab <- bounds_from_setmargin(set_margins[e], b1, b2)
      a_int[e] <- ab[1]
      b_int[e] <- ab[2]
    }
  }
...
likelihood_analitico <- function(teams,result,p_draw,gr){
    d = gr$d[[1]]@prior
    # If ordinal bounds are present, use interval truncation (second-order preference).
    # Otherwise fall back to the original draw/win truncation (baseline behaviour).
    if (!is.null(gr$a) && !is.null(gr$b) && !is.na(gr$a[1]) && !is.na(gr$b[1])) {
      mu_sigma_trunc = trunc_interval(d@mu, d@sigma, gr$a[1], gr$b[1])
    } else {
...
compute_likelihoods <- function(teams, result, p_draw, set_margins = NULL, b1 = NA, b2 = NA){
  gr = graphical_model(teams, result, p_draw, set_margins = set_margins, b1 = b1, b2 = b2)
    if (length(teams)>2){
  ...
  iteration = function(from_ =1){
    for (e in seq(from_,length(events))){
      ...
      g = Game(
        ...
        result,
        p_draw,
        set_margins = if (!is.null(set_margins)) set_margins[e] else NULL,
        b1 = b1,
        b2 = b2
      )
...
History$methods(
  initialize = function(composition, results=list(), times=c(), priors=hash(),
                        mu=MU, sigma=SIGMA, beta=BETA, gamma=GAMMA, p_draw=P_DRAW,
                        set_margins = NULL, b1 = NA, b2 = NA,
    ...
    for (a in this_agents ){
     ...
     agents <<- agents_
     time <<- length(times) > 0
     mu <<- mu; sigma <<- sigma; beta <<- beta; gamma <<- gamma; p_draw <<- p_draw; h_epsilon <<- epsilon; h_iterations <<- iterations; set_margins <<- set_margins; b1 <<- b1; b2 <<- b2
    ...
    while (i <= size){
      ...
      if (length(results)>0){
        ...
        b = Batch(
          ...
          agents,
          p_draw,
          set_margins = if (!is.null(set_margins)) set_margins[idx] else NULL,
          b1 = b1,
          b2 = b2
        )
