#Accuracy — TTT (from CSV skills): 62.46% | Bookmakers: 68.37% (same 1132 matches)
...
cat(sprintf(
  "Accuracy — TTT (from CSV skills): %.2f%% | Bookmakers: %.2f%% (same %d matches)\n",
  ttt_acc, book_acc, n_comp
))

disagree_df <- comp_df %>% filter(bookie_correct & !ttt_correct)
write_csv(disagree_df, "bet365_right_ttt_wrong_BASELINEv5.csv")
...
write_csv(accuracy_summary, "accuracy_summary_BASELINEv5.csv")

cat("Saved:\n")
cat(" - bet365_right_ttt_wrong_BASELINEv5.csv\n")
cat(" - accuracy_summary_BASELINEv5.csv\n")
