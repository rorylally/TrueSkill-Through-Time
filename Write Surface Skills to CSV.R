...
# ----------------------------
# OUTPUT FILE (edit name if you want)
# ----------------------------
OUT_CSV <- "surface_ttt_preds_byATPv2.csv"
...
if (file.exists(OUT_CSV)) {
  ...
  # ---- All 2024 ATP codes available (1st Round only) ----
  ks <- df0 %>%
  ...
  for (k in ks) {
    ...
    # --- Build TTT training comps using (player_surface) node id ---
  ...
  # ---- SAVE to CSV (THIS IS THE KEY BIT) ----
