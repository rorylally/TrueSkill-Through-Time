...
A_SCALE_GRID <- c(0.2, 0.3, 0.4, 0.5)  
W_CAP <- 4L                                           
...
# ============================
# 2) Runner function: runs YOUR rolling eval for one a_scale
# ============================
run_one_scale <- function(a_scale, return_outputs = FALSE) {
  ...
  for (k in ks) {
    ...
    # --- Add margin & weight columns to current training pool ---
    # ORIGINAL weight0 = clamp(1 + set_margin, 1, 4)
    # NEW weight = clamp(round(weight0 * a_scale), 1, 4)
    train_pool <- train_pool %>%
      mutate(
        ...
        weight0    = pmin(W_CAP, pmax(1L, weight_raw)),              # original capped weight
        
        weight     = as.integer(round(weight0 * a_scale)),           # scaled
        weight     = pmin(W_CAP, pmax(1L, weight))                   # keep cap 1..4
      )
    
    # --- Expand matches according to weight (margin-based influence) ---
    ...
    # --- Fit Global TTT (with weighted history) ---
    m <- History(
      composition = comp,
      times       = times_tr,
      sigma       = 4,
    ...
    # --- Predict test matches + add flags (same as your script) ---
    preds <- test_df %>%
    ...
    # --- EXPAND training pool with tested tournament ---
  ...
  # ---- Apples-to-apples comparison (same matches with usable odds) ----
  comp_df <- all_df %>%
  ...
  # ---- TTT accuracy table by year (same format) ----
  ttt_by_year <- all_df %>%
...
# ============================
# 3) TUNE: run all a_scales, PRINT PROGRESS, store accuracy
# ============================
...
# Write tuning table
write_csv(tuning_results, "ttt_weight_scaling_tuning_accuracy.csv")
cat("\nWrote: ttt_weight_scaling_tuning_accuracy.csv\n")

# ============================
# 3b) PRINT SUMMARY OF ALL VALUES (end)
# ============================
...
# ============================
# 4) FINAL RUN with BEST a_scale + EXPORT (same outputs, tagged)
# ============================
final <- run_one_scale(a_scale = best_a, return_outputs = TRUE)
all_df <- final$all_df

# Print same summary lines
...
print(final$ttt_table)
readr::write_csv(final$ttt_table, "ttt_accuracy_by_year.csv")
...
# Save outputs (tagged by best a_scale)
tag <- sprintf("aScale%.2f", best_a)
tag <- gsub("\\.", "p", tag)  # filenames: 1.25 -> 1p25

write_csv(disagree_df, paste0("bet365_right_ttt_wrong_base_vWeight_", tag, ".csv"))
write.csv(all_df_export, paste0("Global_TTT_rolling_result_base_vWeight_", tag, ".csv"), row.names = FALSE)
write_xlsx(all_df_export, paste0("Global_TTT_rolling_result_base_vWeight_", tag, ".xlsx"))

cat(sprintf("\nFiles written:\n- Global_TTT_rolling_result_base_vWeight_%s.[csv/xlsx]\n- bet365_right_ttt_wrong_base_vWeight_%s.csv\n",
            tag, tag))

# ============================
# 5) Bet365 directional accuracy (same as your script; unchanged)
# ============================
...
write_csv(acc_table, paste0("bet365_directional_accuracy_base_vWeight_", tag, ".csv"))
cat(sprintf("\nWrote: bet365_directional_accuracy_base_vWeight_%s.csv\n", tag))

# (Optional) Quick confirmation of which a_scale produced the exported all_df:
