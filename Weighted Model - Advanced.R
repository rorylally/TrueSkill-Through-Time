...
library(dplyr)
library(purrr)
library(TrueSkillThroughTimeOrdinal)   # <-- NEW
library(tidyr)
library(lubridate)
library(stringr)
library(writexl)
...
# Ordinal (second-order preference) thresholds
# Start with your working values; later you should tune these.
B1 <- 1
B2 <- 1.1
...
# ---- Seed training with 2023 (top 100 only, same as your test restriction) ----
train_pool <- df0 %>%
  filter(
    year(Date) == 2023,
...
for (k in ks) {
  ...
  # --- Build training composition AND set_margins in the SAME ROW ORDER ---
  ...
  # This must align 1-to-1 with comp (i.e., one entry per training match)
  set_margins_tr <- train_pool$margin
  
  # --- Fit Ordinal TTT (second-order preference) ---
  m <- History(
    composition = comp,
    times       = times_tr,
    sigma       = 4,
  ...
  preds <- test_df %>%
    ...
    mutate(
      ...
      # beta-correct win probability (your same method)
      ...
      # Bet365 pick and correctness (shorter odds favourite)
    ...
    select(
      ...
      Winner, Loser,
      margin,                      # <-- optional: keep for debugging/analysis
  ...
  # Keep it consistent: you’re modelling top-100, so only add those matches.
...
cat(sprintf("Accuracy — TTT (ordinal): %.2f%% | Bet365: %.2f%% (same %d matches)\n",
            ttt_acc, book_acc, n_comp))

# --- TTT accuracy tables ---
overall_acc <- mean(all_df$ttt_correct, na.rm = TRUE) * 100
cat(sprintf("2024 ATP Global model test accuracy (ordinal; expanding from 2023 R1): %.1f%%\n", overall_acc))
...
cat("\nTTT ACCURACY (ORDINAL)\n")
print(ttt_table)
readr::write_csv(ttt_table, "ttt_accuracy_by_year_ordinal.csv")
...
write_csv(disagree_df, "bet365_right_ttt_wrong_ordinal.csv")
write.csv(all_df_export, "Global_TTT_rolling_result_ordinal.csv", row.names = FALSE)
write_xlsx(all_df_export, "Global_TTT_rolling_result_ordinal.xlsx")
cat("Files written: Global_TTT_rolling_result_ordinal.[csv/xlsx] and bet365_right_ttt_wrong_ordinal.csv\n")
...
cat(sprintf("Accuracy — TTT (ordinal): %.2f%% | Bet365: %.2f%% (same %d matches)\n",
            ttt_acc, book_acc, n_comp))
write_csv(acc_table, "bet365_directional_accuracy_ordinal.csv")
